...
const DataDetailScreen = () => {
  ...
  useFocusEffect(
    React.useCallback(() => {
      ...
      if (route?.params?.profileData?.id) {
        ...
        console.log("Found valid appointment ID from appointment prop:", appointment.id);
        getGraphData();
      } else {
  ...
  const getGraphData = async (openValue) => {
    try {
      ...
      // Get appointment ID from either route params or appointment prop
      let appointmentId = null;
      if (route?.params?.profileData?.id) {
        appointmentId = route.params.profileData.id;
      } else if (appointment?.id) {
  ...
  const renderChart = (index) => {
    ...
    // For timer type, divide by 100 to make the values more manageable for display
    const displayData = item.type === "timer" ? chartData.map((val) => val / 100) : chartData;

    // Determine the dynamic width based on data points (minimum width is screen width)
        ...
        {loading ? (
          ...
          <View style={styles.infoBox}>
            ...
            <View style={styles.row}>
              ...
              <Text style={styles.value}>{profileData.start_time || "N/A"}</Text>
            </View>
            <View style={styles.row}>
              <Text style={styles.label}> End Time:</Text>
              <Text style={styles.value}>{profileData.end_time || "N/A"}</Text>
            </View>
          </View>
        ) : (
          ...
          {tableData.length > 0 ? (
            ...
            <View style={styles.noDataContainer}>
              ...
              <Text style={styles.noDataSubText}>
                {!route?.params?.profileData?.id && !appointment?.id
...
const styles = StyleSheet.create({
  ...
  contentContainer: {
    flex: 1,
    marginBottom: 60, // Add bottom margin to prevent content from being hidden by bottom nav
  },
  loader: {
    marginTop: 50,
  },
